...
def new_reference(temporary_reference_list, temporary_citation_list):
    ...
    match reference_type:
        case 'website':
            ...
            reference, citation = website_reference_builder(author_name, title, date_published, date_accessed, website_name, URL, author_name_citation)
        case 'book':
            chapter_title, edition, volume_number, publisher, publisher_place, page_range = book_reference_details_inputs()

            reference, citation = book_reference_builder(author_name, title, date_published, date_accessed, URL, chapter_title, edition, volume_number, publisher, publisher_place, page_range, author_name_citation)
        case 'journal':
            journal_name, volume_number, issue_number, page_range = journal_reference_details_inputs()

            reference, citation = journal_reference_builder(author_name, title, date_published, date_accessed, URL, journal_name, volume_number, issue_number, page_range, author_name_citation)
        case 'video':
